...
ALTER TABLE public.attendance ADD CONSTRAINT unique_attendance_per_day UNIQUE (institute_id, student_id, date);
...
-- Base Policy Example: Super Admins bypass, regular users scoped to institute
...
CREATE POLICY "Teachers isolation policy" ON public.teachers
    FOR ALL
    USING (
        public.is_super_admin() OR 
        institute_id = public.get_auth_user_institute_id()

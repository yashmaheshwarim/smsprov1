...
export default function StudentNotificationsScreen() {
  ...
  const instId = ''; // Students don't have instituteId directly, we'll use the student's batch to infer
  ...
  // Resolve the student's institute ID from their batch/student record
  useEffect(() => {
    const resolveInstitute = async () => {
      ...
      try {
        ...
        // fallback - try the batch mapping
        if (studentBatch) {
          try {
            const { data: batchData } = await supabase
              .from('batches')
              .select('institute_id')
              .eq('name', studentBatch)
              .single();
            if (batchData?.institute_id) {
              setStudentInstId(batchData.institute_id);
            }
          } catch {}
        }
      }
  ...
  const loadNotifications = useCallback(async () => {
    if (!studentInstId || !userId) return;
    try {
      const data = await fetchNotifications(studentInstId, 'student', userId);
  ...
  }, [studentInstId, loadNotifications]);
  ...
  const handleMarkRead = async (notifId: string) => {
    if (!studentInstId) return;
    const success = await markNotificationAsRead(notifId, userId, 'student');
    if (success) {
      setNotifications((prev) =>
  ...
  const handleMarkAllRead = async () => {
    if (!studentInstId) return;
    const success = await markAllNotificationsAsRead(studentInstId, userId, 'student');
    if (success) {
      setNotifications((prev) =>
    ...
    >
      ...
      {!studentInstId ? (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyText}>Unable to load notifications</Text>
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubtext}>
            Your institute will send you important updates here
          </Text>
        </View>
      ) : (
...
const styles = StyleSheet.create({
  ...
  markAllText: { fontSize: 12, fontWeight: '600', color: '#1e40af' },
